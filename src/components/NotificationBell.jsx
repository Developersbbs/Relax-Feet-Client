  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="relative inline-flex items-center justify-center rounded-full p-2 sm:p-3 text-[#0099CC] hover:bg-[#f0d6d6] hover:scale-105 active:scale-95 transition-all duration-200 touch-manipulation min-h-[44px] min-w-[44px] focus:outline-none focus:ring-2 focus:ring-[#007aa3] focus:ring-opacity-50"
        aria-label={`Notifications ${totalUnread > 0 ? `(${totalUnread} unread)` : '(no unread)'}`}
      >
        <HiBell className="h-5 w-5 sm:h-6 sm:w-6" />
        {totalUnread > 0 && (
          <span className="absolute -top-1 -right-1 sm:-top-0.5 sm:-right-0.5 inline-flex h-5 min-w-[20px] sm:h-4 sm:min-w-[16px] items-center justify-center rounded-full bg-[#0099CC] px-1 sm:px-0.5 text-[10px] sm:text-[9px] font-bold text-white animate-pulse">
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-72 sm:w-80 max-w-[calc(100vw-1rem)] sm:max-w-sm mx-2 sm:mx-0 overflow-hidden rounded-xl border border-[#e0f5fb] bg-white shadow-2xl z-50">
          {/* Header */}
          <div className="flex items-center justify-between px-3 sm:px-4 py-3 border-b border-slate-100 bg-gradient-to-r from-blue-50 to-indigo-50">
            ...
            <Link
              to="/notifications"
              className="text-xs sm:text-sm font-medium text-[#0099CC] hover:text-[#007aa3] hover:bg-[#007aa3] px-2 py-1 rounded-md transition-colors ml-2 flex-shrink-0"
          ...
          {isLoading ? (
            <div className="flex items-center justify-center py-6 sm:py-8 px-4">
              <div className="flex flex-col items-center gap-3">
                <div className="h-6 w-6 sm:h-7 sm:w-7 animate-spin rounded-full border-2 border-[#0099CC] border-t-transparent"></div>
            ...
            notifications.map((item) => (
              <div key={item._id} className="border-b border-slate-50 last:border-b-0">
                  <button
                    ...
                    className={`flex w-full flex-col items-start gap-2 px-3 sm:px-4 py-3 sm:py-4 text-left transition-all hover:bg-[#007aa3] hover:shadow-sm touch-manipulation min-h-[60px] sm:min-h-[56px] active:scale-95 ${
                    ...
                    <div className="flex items-center justify-between w-full text-xs text-slate-500">
                      ...
                      {loadingActions.has(item._id) ? (
                        <div className="flex items-center gap-1 text-blue-400">
                          <div className="w-3 h-3 border border-[#0099CC] border-t-transparent rounded-full animate-spin"></div>
                          <span className="text-xs">Reading...</span>
                        </div>
                      ) : (
                        <span className="text-[#0099CC] font-medium ml-2 flex-shrink-0">Mark Read</span>
                      )}
                    </div>
                  </button>
                </div>
